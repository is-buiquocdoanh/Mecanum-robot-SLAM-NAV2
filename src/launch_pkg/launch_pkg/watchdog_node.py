...
class MultiNodeWatchdog(Node):
	def __init__(self):
		...
		self.is_recv_rtc = False
		self.is_recv_rfid = False
		self.is_recv_magline = False
	...
	def generate_callback(self, node_name, topic):
		def callback(msg):
			...
			if node_name == 'sti_rfid':
				self.is_recv_rfid = True

			elif node_name == 'ros_serial_bridge':
				self.is_recv_rtc = True

			elif node_name == 'sti_magnetic':
				self.is_recv_magline = True
	...
	def check_all_nodes(self):
		if self.process == 0: # check all data
			print("wait data")
			ck = 0
			if self.is_recv_rfid:
			    ck += 1
			if self.is_recv_rtc:
			    ck += 1
			if self.is_recv_magline:
			    ck += 1
			if ck == 3:
