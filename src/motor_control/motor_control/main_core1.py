...
class IO_PIN():
    def __init__(self):
        # encoder use
        self.ENC_IN_LEFT_A = 14
        self.ENC_IN_LEFT_B = 15

        self.ENC_IN_RIGHT_A = 4
        self.ENC_IN_RIGHT_B = 17
...
class MAIN_CORE(LifecycleNode):
    def __init__(self):
        ...
        self.create_subscription(Int16, "/left_front_query", self.callback_leftwheelquery, 1)
        self.data_leftwheel_query = Int16()
        self.ctime_recv_leftwheel_query = 0
        self.is_recv_leftwheel_query = 0 

        self.create_subscription(Int16, "/right_front_query", self.callback_rightwheelquery, 1)
        self.data_rightwheel_query = Int16() 
        self.is_recv_rightwheel_query = 0
        self.ctime_recv_rightwheel_query = 0

        # Publish data
        self.pub_leftTicks = self.create_publisher(Int16, "/left_front_ticks", 10)
        self.left_wheel_tick_count = Int16()

        self.pub_rightTicks = self.create_publisher(Int16, "/right_front_ticks", 10)
        self.right_wheel_tick_count = Int16()

        self.pub_leftwheel_vel = self.create_publisher(Float64, "/left_front_vel", 10)
        self.left_wheel_vel = Float64()

        self.pub_rightwheel_vel = self.create_publisher(Float64, "/right_front_vel", 10)
        self.right_wheel_vel = Float64()
        # -- 

        self.pub_coreData = self.create_publisher(Twist, "/core_front_info", 10)
        ...
        # print(self.main_pin.ENC_IN_LEFT_A, self.main_pin.ENC_IN_LEFT_B)
    ...
    def left_wheel_tick(self, data):
        print("Left wheel tick detected")
    ...
    def right_wheel_tick(self, data):
        print("right wheel tick detected")
    ...
    def run(self):
        ...
        # current_state = GPIO.input(self.main_pin.ENC_IN_LEFT_A)
        # if current_state != self.last_a_state_left:
        #     if GPIO.input(self.main_pin.ENC_IN_LEFT_B) != current_state:
        #         self.left_wheel_tick_count.data += 1
        #     else:
        #         self.left_wheel_tick_count.data -= 1

        #     # print(f"Position: {self.left_wheel_tick_count.data}")

        # last_state = current_state
        # time.sleep(0.001)  # delay nhỏ để tránh chiếm CPU 100%
